...
response_t UART2_set_baud(void) {
    UART_read(U1SELECT, (uint8_t *const)&U2BRG, sizeof(U2BRG), NULL);
    return SUCCESS;
}

response_t UART2_set_mode(void) {
    uint16_t mode = 0;
    UART_read(U1SELECT, (uint8_t *const)&mode, sizeof(mode), NULL);
...
response_t UART_Passthrough(void) {
    uint16_t baud = 0;
    UART_read(U1SELECT, (uint8_t *const)&baud, sizeof(baud), NULL);
