...
static enum Status configure(enum SPI_SCLK const sclk)
{
    enum Status status = E_OK;
    if ( (status = SPI_set_clock(sclk)) ) { return status; }
    if ( (status = SPI_set_mode(SPI_CPOL_HIGH, SPI_CKE_LEADING)) ) {
        return status;
    }
    if ( (status = SPI_set_word_size(SPI_WORD_SIZE_8)) ) { return status; }
    return status;
}

static enum Status dummy_read(uint8_t num_bytes)
{
    uint8_t dummy = 0xFF;
    enum Status status = E_OK;

    for (uint8_t i = 0; i < num_bytes; ++i) {
        if ( (status = SPI_exchange(&dummy, NULL, sizeof(dummy))) ) {
            return status;
        }
    }
...
static enum Status write_single(uint8_t data_byte)
{
    return SPI_exchange(&data_byte, NULL, sizeof(data_byte));
}

static enum Status read_single(uint8_t *const data_byte)
{
    // The value 0xFF was consistently used for dummy data throughout this
    // driver pre-rewrite. Unknown if that specific value is really necessary.
    uint8_t dummy = 0xFF;
    return SPI_exchange(&dummy, data_byte, sizeof(dummy));
}

static enum Status deselect_media(void)
{
    enum Status status = E_OK;
    if ( (status = SPI_close()) ) { return status; }
    if ( (status = SPI_open(SPI_CS_NONE)) ) { return status; }
    return status;
}

static enum Status deselect_dummy_read_and_close(uint8_t num_bytes)
{
    enum Status status = E_OK;
    if ( (status = deselect_media()) ) { goto close; }
    if ( (status = dummy_read(num_bytes)) ) { goto close; }
...
enum AsyncReadState {
    SD_ASYNC_READ_COMPLETE = 0x00,
    // Initialize to this to start a read sequence.
    SD_ASYNC_READ_QUEUED = 0x01,
    SD_ASYNC_READ_WAIT_START_TOKEN = 0x03,
    SD_ASYNC_READ_NEW_PACKET_READY = 0x02,
    SD_ASYNC_READ_ABORT = 0xFE,
    SD_ASYNC_READ_ERROR = 0xFF
};

enum AsyncWriteState {
    SD_ASYNC_WRITE_COMPLETE = 0x00,
    // Initialize to this to start a write sequence.
    SD_ASYNC_WRITE_QUEUED = 0x01,
    SD_ASYNC_WRITE_TRANSMIT_PACKET = 0x02,
    SD_ASYNC_WRITE_MEDIA_BUSY = 0x03,
    SD_ASYNC_STOP_TOKEN_SENT_WAIT_BUSY = 0x04,
    SD_ASYNC_WRITE_ABORT = 0xFE,
    SD_ASYNC_WRITE_ERROR = 0xFF
};

enum MediaBlockSize {
    // Should always be 512 for v1 and v2 devices.
    SD_MEDIA_BLOCK_SIZE = 512u
};
...
//              These macros indicate to the SD_send_cmd function which element of the sdmmc_cmdtable array
 ...
 * Private Prototypes
 *****************************************************************************/
static SD_RESPONSE SD_send_cmd(uint8_t cmd, uint32_t address);
static enum AsyncWriteState SD_SPI_async_write_tasks(struct SD_ASYNC_IO* info);
static enum AsyncReadState SD_SPI_async_read_tasks(struct SD_ASYNC_IO* info);

/******************************************************************************
 * Public Functions
 *****************************************************************************/
bool SD_SPI_is_media_present(void) {
...
enum Status SD_SPI_sector_read(
    ...
    for (size_t i = 0; i < sector_count; ++i) {
        //Initialize info structure for using the SD_SPI_async_read_tasks() function.
        ...
        if ( (status = configure(SPI_SCLK_10666666)) ) { return status; }
        if ( (status = SPI_open(SPI_CS_SD)) ) { return status; }

        while (1) {
            switch (SD_SPI_async_read_tasks(&info)) {
...
enum Status SD_SPI_sector_write(
    uint32_t sector_address,
    ...
    for (size_t i = 0; i < sector_count; ++i) {
        ...
        while (1) {
            switch (SD_SPI_async_write_tasks(&info)) {
            case SD_ASYNC_WRITE_COMPLETE:
                status = E_OK;
                break;
            case SD_ASYNC_WRITE_ERROR:
                status = E_FAILED;
                break;
            default:
                status = E_FAILED;
...
bool SD_SPI_is_media_initialized(void) {
    return (mediaInformation.state != SD_STATE_NOT_INITIALIZED);
}

bool SD_SPI_is_write_protected(void) {
    return SD_SPI_GetWriteProtect() ? true : false;
}

enum Status SD_SPI_media_initialize(void)
{
    uint16_t timeout;
    ...
    // MMC media powers up in the open-drain mode and cannot handle a clock
    // faster than 400kHz. Initialize SPI port to <= 400kHz
    if ( (status = configure(SPI_SCLK_125000)) ) { return status; }
    if ( (status = SPI_open(SPI_CS_SD)) ) { return status; }

    // Media wants the longer of: Vdd ramp time, 1 ms fixed delay, or 74+ clock
    // pulses. According to spec, CS should be high during the 74+ clock
    // pulses. In practice it is preferable to wait much longer than 1ms, in
    // case of contact bounce, or incomplete mechanical insertion (by the time
    // we start accessing the media).
    DELAY_ms(SD_SPI_STARTUP_DELAY_MS);

    // Send CMD0 (with CS = 0) to reset the media and put SD cards into SPI
    // mode.
    timeout = 100;

    do {
        // Toggle chip select, to make media abandon whatever it may have been
        // doing before. This ensures the CMD0 is sent freshly after CS is
        // asserted low, minimizing risk of SPI clock pulse master/slave
        // synchronization problems, due to possible application noise on the
        // SCK line.
        SPI_open(SPI_CS_NONE);
        ...
        response = SD_send_cmd(SD_GO_IDLE_STATE, 0x0);
    } while ((response.r1._byte != 0x01) && (timeout != 0));

    // Check if all attempts failed and we timed out.  Normally, this won't
    // happen, unless maybe the SD card was busy, because it was previously
    // performing a read or write operation, when it was interrupted by the
    // microcontroller getting reset or power cycled, without also resetting or
    // power cycling the SD card. In this case, the SD card may still be busy
    // (ex: trying to respond with the read request data), and may not be ready
    // to process CMD0.  In this case, we can try to recover by issuing CMD12
    // (STOP_TRANSMISSION).
    if (timeout == 0) {
        SPI_close();
        SPI_open(SPI_CS_NONE);
        ...
        // Send CMD12, to stop any read/write transaction that may have been in
        // progress. Blocks until SD card signals non-busy.
        (void) SD_send_cmd(SD_STOP_TRANSMISSION, 0x0);
        //Now retry to send send CMD0 to perform software reset on the media.
        response = SD_send_cmd(SD_GO_IDLE_STATE, 0x0);

        // Check if card in idle state now.
        if (response.r1._byte != 0x01) {
            // Card failed to process CMD0 yet again.  At this point, the
            // proper thing to do would be to power cycle the card and retry,
            // if the host circuitry supports disconnecting the SD card power.
            // Since the SD/MMC PICtail+ doesn't support software controlled
            // power removal of the SD card, there is nothing that can be done
            // with this hardware. Therefore, we just give up now.  The user
            // needs to physically power cycle the media and/or the whole
            // board.
    ...
    // operating voltage range of the host, then it may not respond. If it does
    // support the range, it will respond with a type R7 response packet
    // (6 bytes/48 bits). Additionally, if the SD card is MMC or SD card v1.
    // spec device, then it may respond with invalid command.  If it is a v2.0
    // spec SD card, then it is mandatory that the card respond to CMD8.
    // Note: If changing "0x1AA", CRC value in table must also change.
    response = SD_send_cmd(SD_SEND_IF_COND, 0x1AA);

    if (
        ((response.r7.bytewise.argument._returnVal & 0xFFF) == 0x1AA)
        && (!response.r7.bitwise.bits.ILLEGAL_CMD)
    ) {
        // If we get to here, the device supported the CMD8 command and didn't
        // complain about our host voltage range. Most likely this means it is
        // either a v2.0 spec standard or high capacity SD card (SDHC)

        // Send CMD58 (Read OCR [operating conditions register]). Response type
        // is R3, which has 5 bytes. uint8_t 4 = normal R1 response uint8_t,
        // bytes 3-0 are = OCR register value.
        response = SD_send_cmd(SD_READ_OCR, 0x0);

        // Now that we have the OCR register value in the response packet, we
        // could parse the register contents and learn what voltage the SD card
        // wants to run at. If our host circuitry has variable power supply
        // capability, it could theoretically adjust the SD card Vdd to the
        // minimum of the OCR to save power.

        // Now send CMD55/ACMD41 in a loop, until the card is finished with its
        // internal initialization.
        // Note: SD card specs recommend >= 1 second timeout while waiting for
        // ACMD41 to signal non-busy.
        for (timeout = 0; timeout < 0xFFFF; timeout++) {
            // Send CMD55 (lets SD card know that the next command is
            // application specific (going to be ACMD41)).
            (void) SD_send_cmd(SD_APP_CMD, 0x00000000);

            // Send ACMD41.  This is to check if the SD card is finished
            // booting up/ready for full frequency and all further commands.
            // Response is R3 type (6 bytes/48 bits, middle four bytes contain
            // potentially useful data).
            // Note: When sending ACMD41, the HCS bit is bit 30, and must be
            // set to tell SD card the host supports SDHC
            response = SD_send_cmd(SD_SD_SEND_OP_COND, 0x40000000);

            // The R1 response should be = 0x00, meaning the card is now in the
            // "standby" state, instead of the "idle" state (which is the
            // default initialization state after CMD0 reset is issued). Once
            // in the "standby" state, the SD card is finished with basic
            // initialization and is ready for read/write and other commands.
            if (response.r1._byte == 0) {
                // Card is finished initializing.
                break;
            }
        }
        ...
        // Now check the CCS bit (OCR bit 30) in the OCR register, which is in
        // our response packet. This will tell us if it is a SD high capacity
        // (SDHC) or standard capacity device.
        ...
        // SD Card should now be finished with initialization sequence. Device
        // should be ready for read/write commands.
        ...
        // The CMD8 wasn't supported.  This means the card is definitely not a
        // v2.0 SDHC card.
        mediaInformation.gSDMode = SD_MODE_NORMAL;
        ...
        do {
            ...
            // When argument is 0x00000000, this queries MMC cards for
            // operating voltage range.
            response = SD_send_cmd(SD_SEND_OP_COND, 0x00000000);
            timeout--;
        ...
        if (timeout == 0) {
            ...
            // since all cards v1 and v2 are required to support 512 byte block
            // size, and this is supposed to be the default size selected on
            // cards that support other sizes as well.
            // Set read/write block length to 512 bytes.
            // response = SD_send_cmd(SET_BLOCKLEN, 0x00000200);
        }
    }
    ...
    do {
        ...
        response = SD_send_cmd(SD_SEND_CSD, 0x00);
        timeout--;
    } while ((response.r1._byte != 0x00) && (timeout != 0));
    ...
    uint8_t csd_response[20];
    memset(csd_response, 0xFF, sizeof(csd_response));
    SPI_exchange(csd_response, csd_response, sizeof(csd_response));

    // Hopefully the first byte is the datatoken, however, some cards do not
    // send the response token before the CSD register.
    if (csd_response[0] == SD_TOKEN_START) {
        // As the first byte was the datatoken, we can drop it.
        memcpy(csd_response, csd_response + 1, sizeof(csd_response) - 1);
    }
    ...
    // Calculate the finalLBA (see SD card physical layer simplified spec 2.0,
    // section 5.3.2). In USB mass storage applications, we will need this
    // information to correctly respond to SCSI get capacity requests.
    // Note: method of computing finalLBA depends on CSD structure spec version
    // (either v1 or v2).
    // Check CSD_STRUCTURE field for v2+ struct device.
    if (csd_response[0] & 0xC0) {
        // Must be a v2 device (or a reserved higher version, that doesn't
        // currently exist)

        // Extract the C_SIZE field from the response.  It is a 22-bit number
        // in bit position 69:48.  This is different from v1. It spans bytes 7,
        // 8, and 9 of the response.
        c_size = (
            (((uint32_t) csd_response[7] & 0x3F) << 16)
            | ((uint16_t) csd_response[8] << 8)
            | csd_response[9]
        );

        // -1 on end is correction factor, since LBA = 0 is valid.
        mediaInformation.finalLBA = (
            ((uint32_t)(c_size + 1) * (uint16_t)(1024u)) - 1
        );
    } else {
        // Check CSD_STRUCTURE field for v1 struct device.
        // Must be a v1 device.
        // Extract the C_SIZE field from the response.  It is a 12-bit number
        // in bit position 73:62.
        // Although it is only a 12-bit number, it spans bytes 6, 7, and 8,
        // since it isn't byte aligned.
        c_size = (
            ((uint32_t)csd_response[6] << 16)
            | ((uint16_t)csd_response[7] << 8)
            | csd_response[8]
        );
        // Clear all bits that aren't part of the C_SIZE.
        c_size &= 0x0003FFC0;
        //  Shift value down, so the 12-bit C_SIZE is properly right justified
        // in the uint32_t.
        c_size = c_size >> 6;

        // Extract the C_SIZE_MULT field from the response.  It is a 3-bit
        // number in bit position 49:47.
        c_size_mult = (
            ((uint16_t)((csd_response[9] & 0x03) << 1))
            | ((uint16_t)((csd_response[10] & 0x80) >> 7))
        );

        // Extract the BLOCK_LEN field from the response. It is a 4-bit number
        // in bit position 83:80.
        block_len = csd_response[5] & 0x0F;

        // -9 because we report the size in sectors of 512 bytes each.
        block_len = 1 << (block_len - 9);

        // Calculate the finalLBA (see SD card physical layer simplified spec
        // 2.0, section 5.3.2). In USB mass storage applications, we will need
        // this information to correctly respond to SCSI get capacity requests
        // (which will cause SD_capacity_read() to get called).
        mediaInformation.finalLBA = (
            // -1 on end is correction factor, since LBA = 0 is valid.
            ((c_size + 1) * (1 << (c_size_mult + 2)) * block_len) - 1
        );
    }
    ...
    (void) SD_send_cmd(SD_CRC_ON_OFF, 0x0);

    // Now set the block length to media sector size. It should be already set
    // to this.
    (void) SD_send_cmd(SD_SET_BLOCK_LENGTH, mediaInformation.sectorSize);
    ...
    switch (info->bStateVariable) {
        ...
        case SD_ASYNC_READ_QUEUED:
            ...
            if (ioInfo.dwBytesRemaining <= SD_MEDIA_BLOCK_SIZE) {
                ...
                response = SD_send_cmd(SD_READ_SINGLE_BLOCK, ioInfo.dwAddress);
            } else {
                SingleBlockRead = false;
                response = SD_send_cmd(SD_READ_MULTI_BLOCK, ioInfo.dwAddress);
            }
        ...
        case SD_ASYNC_READ_NEW_PACKET_READY:
            ...
            if (ioInfo.dwBytesRemaining != 0x00000000) {
                ...
                SPI_exchange(ioInfo.pBuffer, ioInfo.pBuffer, ioInfo.wNumBytes);
                ...
                if (SingleBlockRead == false) {
                    (void) SD_send_cmd(SD_STOP_TRANSMISSION, 0x00000000);
                }
                deselect_dummy_read_and_close(1);
        ...
        case SD_ASYNC_READ_ABORT:
            ...
            response = SD_send_cmd(SD_STOP_TRANSMISSION, 0x00000000);
    ...
    switch (info->bStateVariable) {
        ...
        case SD_ASYNC_WRITE_QUEUED:
            ...
            if (ioInfo.dwBytesRemaining <= SD_MEDIA_BLOCK_SIZE) {
                ...
                response = SD_send_cmd(SD_APP_CMD, 0x00000000); //Send CMD55
                if (response.r1._byte == 0x00) //Check if successful.
                {
                    SD_send_cmd(SD_SET_WRITE_BLOCK_ERASE_COUNT, preEraseBlockCount); //Send ACMD23
                }
            }
            ...
            response = SD_send_cmd(command, ioInfo.dwAddress);
            ...
            return SD_ASYNC_WRITE_TRANSMIT_PACKET;

        case SD_ASYNC_WRITE_TRANSMIT_PACKET:
            // Check if we just finished programming a block, or we are
            // starting for the first time.  In this case, we need to send the
            // data start token.
            if (blockCounter == SD_MEDIA_BLOCK_SIZE) {
                // Send the correct data start token, based on the type of
                // write we are doing.
                if (command == SD_WRITE_MULTI_BLOCK) {
                    write_single(SD_TOKEN_START_MULTI_BLOCK);
                } else { // Single block write.
                    write_single(SD_TOKEN_START);
                }
            }
            ...
            if (blockCounter == 0) {
                ...
                if ((token & SD_WRITE_RESPONSE_TOKEN_MASK) != SD_TOKEN_DATA_ACCEPTED) {
                    ...
                    return SD_ASYNC_WRITE_MEDIA_BUSY;
                }
        ...
        case SD_ASYNC_WRITE_MEDIA_BUSY:
            if (WriteTimeout != 0) {
                WriteTimeout--;
                dummy_read(1); //Dummy read to gobble up a uint8_t (ex: to ensure we meet NBR timing parameter)
                read_single(&data_byte); //Poll the media.  Will return 0x00 if still busy.  Will return non-0x00 is ready for next data block.
                if (data_byte != 0x00) {
                    ...
                    if (ioInfo.dwBytesRemaining == 0) {
                        ...
                        if (command == SD_WRITE_MULTI_BLOCK) {
                            ...
                            write_single(SD_TOKEN_STOP_TRANSMISSION);
        ...
        case SD_ASYNC_STOP_TOKEN_SENT_WAIT_BUSY:
            ...
            if (WriteTimeout != 0) {
                ...
                if (data_byte != 0x00) {
                    ...
                    deselect_dummy_read_and_close(1); //NEC timing parameter clocking
                    info->bStateVariable = SD_ASYNC_WRITE_COMPLETE;
                    mediaInformation.state = SD_STATE_READY_FOR_COMMAND; //Free the media for new commands, since we are now done with it
                    return SD_ASYNC_WRITE_COMPLETE;
                }
        ...
        case SD_ASYNC_WRITE_ABORT:
            ...
            (void) SD_send_cmd(SD_STOP_TRANSMISSION, 0x00000000);
            deselect_media();
            dummy_read(1); //After raising CS pin, media may not tri-state data out for 1 bit time.
            info->bStateVariable = SD_ASYNC_WRITE_ERROR;
...
static SD_RESPONSE SD_send_cmd(uint8_t cmd, uint32_t address) {
    ...
    write_single(sdmmc_cmdtable[cmd].CmdCode | SD_COMMAND_TRANSMIT_BIT_MASK);

    write_single(address_bytes[3]);
    write_single(address_bytes[2]);
    write_single(address_bytes[1]);
    write_single(address_bytes[0]);

    write_single(sdmmc_cmdtable[cmd].CRC);
    ...
    if (cmd == SD_STOP_TRANSMISSION) {
        dummy_read(1); //Perform dummy read to fetch the residual non R1 byte
    }
    ...
    if (sdmmc_cmdtable[cmd].responsetype == SD_RESPONSE_R2) {
        ...
        read_single(&response.r2._byte0); //Fetch the second byte of the response.
    ...
    } else if (sdmmc_cmdtable[cmd].responsetype == SD_RESPONSE_R7) //also used for response R3 type
    {
        ...
        read_single(&response.r7.bytewise.argument._byte3);
        read_single(&response.r7.bytewise.argument._byte2);
        read_single(&response.r7.bytewise.argument._byte1);
        read_single(&response.r7.bytewise.argument._byte0);
    }
