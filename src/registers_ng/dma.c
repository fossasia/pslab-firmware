...
static uint16_t *g_initial_stas[CHANNEL_NUMEL] = {NULL, NULL, NULL, NULL};
...
{
    switch (source) {
    case DMA_SOURCE_ADC:; // Semicolon is necessary; see C11 §6.8.1.
        // NOLINTNEXTLINE(readability-isolate-declaration)
        extern uint16_t const volatile ADC1BUF0, ADC1BUF1, ADC1BUF2, ADC1BUF3;
        uint16_t const volatile *const adcbufs[CHANNEL_NUMEL] = {
            [CHANNEL_1] = &ADC1BUF0,
            [CHANNEL_2] = &ADC1BUF1,
            [CHANNEL_3] = &ADC1BUF2,
            [CHANNEL_4] = &ADC1BUF3,
        };
        return (uint16_t)adcbufs[channel];

    case DMA_SOURCE_IC:;
        // NOLINTNEXTLINE(readability-isolate-declaration)
        extern uint16_t const volatile IC1BUF, IC2BUF, IC3BUF, IC4BUF;
        uint16_t const volatile *const icbufs[CHANNEL_NUMEL] = {
            [CHANNEL_1] = &IC1BUF,
            [CHANNEL_2] = &IC2BUF,
            [CHANNEL_3] = &IC3BUF,
            [CHANNEL_4] = &IC4BUF,
        };
        return (uint16_t)icbufs[channel];
...
)
{
    ...
    *regs.p_stal = address;
    *regs.p_stah = 0;
    g_initial_stas[channel] = (uint16_t *)address;
}
...
uint16_t DMA_get_progress(Channel const channel)
{
    struct DMAxRegisters const regs = get_registers(channel);
    uint16_t *const current_sta = (uint16_t *)(*regs.p_stal);
    return current_sta - g_initial_stas[channel];
}
