...
void IC_reset(Channel const channel)
{
    static struct ICConf {
        ...
        .con1bits = {
            .ICM = ICM_NONE,
            .ICTSEL = IC_TIMER_TMR5,
        },
        .con2bits = {
            .ICTRIG = IC_TASK_TRIGGER,
            .SYNCSEL = SYNCSEL_TMR5,
        },
    }; // clang-format on
...
void IC_setup(Channel const channel, IC_Timer const timer)
{
...
void IC_start(Channel const channel, Edge const edge)
{
    ...
    uint16_t const edge2icm = 1;
    get_registers(channel).con1bits_p->ICM = (uint16_t)edge + edge2icm;
}
